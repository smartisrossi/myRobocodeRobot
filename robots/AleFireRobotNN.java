...
import ubc.ecee.cpen502.NeuralNetwork.*;
...
public class AleFireRobotNN extends AdvancedRobot {
	...
	private LogFile logLearningErrorNN;
	private Random r;
	...
	public void run() {
		...
		logLearningErrorNN = new LogFile("LearningErrorNN"+startTime+".txt", this);
		...
		while(true) {
			...
			case action:	
				...
				if(lastAction != null && lastState != null) {
					double[][] trainVector = new double[1][NNinputDimensions];
					double truePosition;
					
					//normalize input values before training the neural network
					//distance
					trainVector[0][0] = lastState[0]/maxDistance;
					//enemy energy
					trainVector[0][1] = lastState[1]/maxEnergy;
					//my energy
					trainVector[0][2] = lastState[2]/maxEnergy;
					//actions
					truePosition = lastState[3];
					...
					labelVector[0] = computeQ(lastState, currentState, currentReward)/maxQ;
					...
					error += Math.pow(mind.outputFor(trainVector[0]) - labelVector[0], 2);
				}
	...
	public void onDeath(DeathEvent event) {
		...
		double[] labelVector = new double[1];
		labelVector[0] = computeQ(lastState, state, currentReward)/maxQ;
		if(stateEqualsTarget(lastState)) {
			double Qdifference = actualQ-newQ;
			logQdifference.reportLog(""+Qdifference);
		}
		...
		//Save the total error at the end of each battle
		double totalError = error/2;
		logLearningErrorNN.reportLog(totalError+"");
